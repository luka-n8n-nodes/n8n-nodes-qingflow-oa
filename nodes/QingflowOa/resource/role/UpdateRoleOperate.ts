...
const UpdateRoleOperate: ResourceOperations = {
	name: '更新角色',
	value: 'updateRole',
	action: '更新角色',
	options: [
		...
		},
		{
			...
			required: true,
			description: '请求体 JSON 格式。必填参数：roleId (int), roleName (string)',
			typeOptions: {
	...
	async call(this: IExecuteFunctions, index: number): Promise<IDataObject | IDataObject[]> {
		...
		if (!body.roleName) {
			throw new Error('请求体中的 roleName 不能为空');
		}
...
export default UpdateRoleOperate;
