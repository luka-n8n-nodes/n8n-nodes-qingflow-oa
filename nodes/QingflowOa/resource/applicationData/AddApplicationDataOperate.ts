...
const AddApplicationDataOperate: ResourceOperations = {
	name: '添加应用数据',
	value: 'addApplicationData',
	action: '添加应用数据',
	options: [
		{
		...
		},
		{
			...
			required: true,
			description:
				'请求体 JSON 格式。必填参数：key (string), path (string), searchHandle (int), searchKey (string), keyGuard (int), ordinal (int), values (array), queryConditions (array), guard (int)',
			typeOptions: {
...
export default AddApplicationDataOperate;
